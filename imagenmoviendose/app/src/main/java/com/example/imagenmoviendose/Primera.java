            ...
            public void run() {
                while(true) {
                        imgMain.post(new Runnable() {
                            @Override
                            public void run() {
                                Picasso.get().load(imagenes[skBar.getProgress()]).into(imgMain);
                            }
                        });
                        if(orientation){
                            imgLeft.post(new Runnable() {
                                @Override
                                public void run() {
                                    if(skBar.getProgress()!=0){
                                        Picasso.get().load(imagenes[skBar.getProgress()-1]).into(imgLeft);
                                    }else{
                            ...
                            imgRight.post(new Runnable() {
                                @Override
                                public void run() {
                                    if(skBar.getProgress()!=17){
                                        Picasso.get().load(imagenes[skBar.getProgress()+1]).into(imgRight);
                                    }else{
